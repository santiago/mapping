...
function startUi() {
    ...
    $("#actions .action").click(function(e) {
        ...
        if ($(this).hasClass('registro')) {
            $(".active.rslide-card").animate({
                right: '-450px'
            });

            $(".login.rslide-card").animate({
                right: '0px'
            }, function() {
...
jQuery(document).ready(function($) {
    function success(pos) {
        ...
        var point = new OpenLayers.LonLat(position[1], position[0]).transform(new OpenLayers.Projection("EPSG:4326"), app.map.getProjectionObject());
